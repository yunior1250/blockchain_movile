...
class AuthService extends ChangeNotifier {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (userResponse.statusCode == 200) {
          ...
          user = User.fromJson(
              userResponse.body); // Aquí estás utilizando User.fromJson

          final userData =  User.fromJson(
              userResponse.body);
            
          await storageWrite(
              token, userData.id, userData.email, userData.username);
  ...
  Future storageWrite(
      String idToken, int? id, String? email, String? username) async {
